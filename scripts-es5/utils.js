 ...
 */friendlyPix.MaterialUtils=function(){function _class(){_classCallCheck(this,_class)}_createClass(_class,null,[{key:'refreshSwitchState',/**
   ...
   */},{key:'upgradeDropdowns',value:function upgradeDropdowns(element){if(element){componentHandler.upgradeElements($('.mdl-js-button',element).get());componentHandler.upgradeElements($('.mdl-js-menu',element).get())}else{componentHandler.upgradeDom()}}/**
   ...
   */},{key:'stopOnEndScrolls',value:function stopOnEndScrolls(){var mdlLayoutElement=$('.mdl-layout');mdlLayoutElement.unbind('scroll')}/**
   * Escapes HTML characters from String.
   */},{key:'escapeHtml',value:function escapeHtml(unsafe){if(!unsafe){return unsafe}return unsafe.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;')}}]);return _class}();
