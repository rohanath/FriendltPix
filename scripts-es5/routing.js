...
page()})}/**
   ...
   */_createClass(_class,[{key:'displayPage',value:function displayPage(attributes,context){var _this2=this;var onlyAuthed=attributes.onlyAuthed;var admin=attributes.admin;if(admin){friendlyPix.Router.enableAdminMode()}else{friendlyPix.Router.disableAdminMode()}if(onlyAuthed){// If the pge can only be displayed if the user is authenticated then we wait or the auth state.
friendlyPix.auth.waitForAuth.then(function(){_this2._displayPage(attributes,context)})}else{this._displayPage(attributes,context)}}},{key:'_displayPage',value:function _displayPage(attributes,context){var onlyAuthed=attributes.onlyAuthed;var pageId=attributes.pageId;if(onlyAuthed&&!firebase.auth().currentUser){pageId='splash';this.splashLogin.show()}friendlyPix.Router.setLinkAsActive(context.canonicalPath);this.pagesElements.each(function(index,element){if(element.id==='page-'+pageId){$(element).show()}else if(element.id==='page-splash'&&onlyAuthed){$(element).fadeOut(1000)}else{$(element).hide()}});friendlyPix.MaterialUtils.closeDrawer();friendlyPix.Router.scrollToTop()}/**
   ...
   */}],[{key:'enableAdminMode',value:function enableAdminMode(){document.body.classList.add('fp-admin')}/**
   * Switch off admin mode in the UI.
   */},{key:'disableAdminMode',value:function disableAdminMode(){document.body.classList.remove('fp-admin')}/**
