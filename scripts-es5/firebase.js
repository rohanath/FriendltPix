...
var userIds=Object.keys(people);userIds.forEach(function(userId){var name=people[userId]._search_index.full_name;var reversedName=people[userId]._search_index.reversed_full_name;if(!name.startsWith(searchString)&&!reversedName.startsWith(searchString)){delete people[userId]}});return people})}/**
   ...
   */},{key:'updatePublicProfile',value:function updatePublicProfile(){var displayName=firebase.auth().currentUser.displayName;var imageUrl=firebase.auth().currentUser.photoURL;// If the main profile Pic is an expiring facebook profile pic URL we'll update it automatically to use the permanent graph API URL.
if(imageUrl&&(imageUrl.indexOf('lookaside.facebook.com')!==-1||imageUrl.indexOf('fbcdn.net')!==-1)){// Fid the user's Facebook UID.
var facebookUID=firebase.auth().currentUser.providerData.find(function(providerData){return providerData.providerId==='facebook.com'}).uid;imageUrl='https://graph.facebook.com/'+facebookUID+'/picture?type=large';firebase.auth().currentUser.updateProfile({photoURL:imageUrl}).then(function(){console.log('User profile updated.')})}if(!displayName){displayName='Anonymous'}var searchFullName=displayName.toLowerCase();var searchReversedFullName=searchFullName.split(' ').reverse().join(' ');try{searchFullName=latinize(searchFullName);searchReversedFullName=latinize(searchReversedFullName)}catch(e){console.error(e)}var updateData={profile_picture:imageUrl||null,full_name:displayName,_search_index:{full_name:searchFullName,reversed_full_name:searchReversedFullName}};return this.database.ref('/people/'+this.auth.currentUser.uid).update(updateData).then(function(){console.log('Public profile updated.')})}/**
...
var likesRef=this.database.ref('likes/'+postId+'/'+this.auth.currentUser.uid);likesRef.on('value',function(data){return callback(!!data.val())});this.firebaseRefs.push(likesRef)}/**
   ...
   */},{key:'addComment',value:function addComment(postId,commentText){var commentObject={text:commentText,timestamp:firebase.database.ServerValue.TIMESTAMP,author:{uid:this.auth.currentUser.uid,full_name:this.auth.currentUser.displayName||'Anonymous',profile_picture:this.auth.currentUser.photoURL||null}};return this.database.ref('comments/'+postId).push(commentObject)}/**
   * Deletes a comment.
   ...
   */},{key:'subscribeToComment',value:function subscribeToComment(postId,commentId,callback){var commentRef=this.database.ref('/comments/'+postId+'/'+commentId);commentRef.on('value',callback);this.firebaseRefs.push(commentRef)}/**
...
var picRef=this.storage.ref(this.auth.currentUser.uid+'/full/'+newPostKey+'/'+fileName);var metadata={contentType:pic.type};var picUploadTask=picRef.put(pic,metadata).then(function(snapshot){console.log('New pic uploaded. Size:',snapshot.totalBytes,'bytes.');return snapshot.ref.getDownloadURL().then(function(url){console.log('File available at',url);return url})}).catch(function(error){console.error('Error while uploading new pic',error)});// Start the thumb file upload to Cloud Storage.
var thumbRef=this.storage.ref(this.auth.currentUser.uid+'/thumb/'+newPostKey+'/'+fileName);var tumbUploadTask=thumbRef.put(thumb,metadata).then(function(snapshot){console.log('New thumb uploaded. Size:',snapshot.totalBytes,'bytes.');return snapshot.ref.getDownloadURL().then(function(url){console.log('File available at',url);return url})}).catch(function(error){console.error('Error while uploading new thumb',error)});return Promise.all([picUploadTask,tumbUploadTask]).then(function(urls){// Once both pics and thumbnails has been uploaded add a new post in the Firebase Database and
// to its fanned out posts lists (user's posts and home post).
var update={};update['/posts/'+newPostKey]={full_url:urls[0],thumb_url:urls[1],text:text,client:'web',timestamp:firebase.database.ServerValue.TIMESTAMP,full_storage_uri:picRef.toString(),thumb_storage_uri:thumbRef.toString(),author:{uid:_this5.auth.currentUser.uid,full_name:_this5.auth.currentUser.displayName||'Anonymous',profile_picture:_this5.auth.currentUser.photoURL||null}};update['/people/'+_this5.auth.currentUser.uid+'/posts/'+newPostKey]=true;update['/feed/'+_this5.auth.currentUser.uid+'/'+newPostKey]=true;return _this5.database.ref().update(update).then(function(){return newPostKey})})}/**
...
updateData['/followers/'+followedUserId+'/'+_this6.auth.currentUser.uid]=follow?!!follow:null;return _this6.database.ref().update(updateData)})}/**
   ...
   */},{key:'toggleBlockUser',value:function toggleBlockUser(followedUserId,block){// Add or remove posts to the user's home feed.
var update={};update['/blocking/'+this.auth.currentUser.uid+'/'+followedUserId]=block?!!block:null;update['/blocked/'+followedUserId+'/'+this.auth.currentUser.uid]=block?!!block:null;return this.database.ref().update(update)}/**
   ...
   */},{key:'registerToBlockedStatusUpdate',value:function registerToBlockedStatusUpdate(userId,callback){var blockStatusRef=this.database.ref('/blocking/'+this.auth.currentUser.uid+'/'+userId);blockStatusRef.on('value',callback);this.firebaseRefs.push(blockStatusRef)}/**
   ...
   */},{key:'reportPost',value:function reportPost(postId){return this.database.ref('/postFlags/'+postId+'/'+this.auth.currentUser.uid).set(true)}/**
   * Flags the comment for inappropriate content.
   */},{key:'reportComment',value:function reportComment(postId,commentId){return this.database.ref('/commentFlags/'+postId+'/'+commentId+'/'+this.auth.currentUser.uid).set(true)}/**
